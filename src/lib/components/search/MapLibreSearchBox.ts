...
export class MapLibreSearchControl implements IControl {
	...
	constructor(options: Partial<MapLibreSearchControlOptions> = {}) {
		...
		this.api = new GeocodingApi(new Configuration({ basePath: options.baseUrl }));
	}
	...
	buildInput(): HTMLElement {
		...
		this.clearButton = inputContainer.appendChild(document.createElement('span'));
		this.clearButton.className = 'cancel hidden';
		this.clearButton.addEventListener('click', this.onClear.bind(this));

		this.loadingSpinner = inputContainer.appendChild(document.createElement('span'));
		...
		this.resultsContainer = container.appendChild(document.createElement('div'));
		this.resultsContainer.className = 'results hidden';
		this.resultsList = this.resultsContainer.appendChild(document.createElement('div'));
		this.resultsList.className = 'results-list';

		const attribution = this.resultsContainer.appendChild(document.createElement('div'));
	...
	async onInput(e: Event, useSearch = false) {
		...
		if (this.lastRequestAt === 0 || timeSinceLastRequest >= this.options.minWaitPeriodMs) {
			if (searchString.length >= this.options.minInputLength) {
				if (searchString !== this.lastRequestString || useSearch) {
					...
					(params as Record<string, unknown>).apiKey = this.options.apiKey;
					...
					try {
						...
						if (useSearch) {
							...
							(searchParams as Record<string, unknown>).apiKey = this.options.apiKey;
	...
	async onSelected(feature: FeaturePropertiesV2) {
		if (!hasGeometry(feature)) {
			...
			const detail = await this.api.placeDetailsV2({
				...
				...(this.options.apiKey ? ({ apiKey: this.options.apiKey } as Record<string, unknown>) : {})
			});
	...
	buildResult(result: FeaturePropertiesV2): HTMLDivElement {
		...
		additionalText.textContent = result.properties.coarseLocation || subtitle(result.properties);
...
function upcastLegacyFeature(feature: GeocodingGeoJSONFeature): FeaturePropertiesV2 {
	return {
		type: 'Feature',
		bbox: feature.bbox,
		...
		properties: {
			...
			context: {
				whosonfirst: {
					...
					continent: extractWofContextcomponent(feature.properties, 'continent'),
					country: extractWofContextcomponent(feature.properties, 'country'),
					county: extractWofContextcomponent(feature.properties, 'county'),
					localadmin: extractWofContextcomponent(feature.properties, 'localadmin'),
					locality: extractWofContextcomponent(feature.properties, 'locality'),
					neighbourhood: extractWofContextcomponent(feature.properties, 'neighbourhood'),
			...
			layer: feature.properties.layer === 'venue' ? 'poi' : feature.properties.layer,
